...
| `/health` | GET | Время работы, версия CLI, во что развернулся каждый алиас, расход подписки |

В ответах стоят настоящий id модели, счётчики токенов с учётом чтения и записи кэша и причина остановки; на стороне OpenAI `max_tokens` превращается в `finish_reason: "length"`. Ошибки приходят в формате того эндпоинта, куда шёл запрос, и с кодом, по которому видно, что случилось:
...
Некоторым полям запроса деваться некуда:

- `tools`. CLI не может вызывать функции, которые живут в клиенте, поэтому прокси их пропускает и пишет предупреждение в лог.
- `max_tokens`, `temperature`, `stop` и прочие параметры генерации. Их CLI выставляет сам.
...
├── conversation.rs  ходы запроса, вход CLI, ключи истории
├── turn.rs          один запрос: продолжить или начать заново, ретранслировать, запомнить
